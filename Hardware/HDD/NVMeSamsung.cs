...
namespace OpenHardwareMonitor.Hardware.HDD {
  internal class NVMeSamsung : INVMeDrive {
    ...
    public bool IdentifyController(SafeHandle hDevice, out Kernel32.NVMeIdentifyControllerData data) {
      ...
      sptwb.Spt.DataIn = Kernel32.SCSI_IOCTL_DATA_OUT;
      sptwb.DataBuf[0] = 1;

      var length = Marshal.SizeOf<Kernel32.SPTWith512Buffer>();
      buffer = Marshal.AllocHGlobal(length);
      Marshal.StructureToPtr(sptwb, buffer, false);
      var validTransfer = Kernel32.DeviceIoControl(hDevice, Kernel32.Command.IOCTL_SCSI_PASS_THROUGH, buffer, length, buffer, length, out _, IntPtr.Zero);
      ...
      if (validTransfer) {
        ...
        length = Marshal.SizeOf<Kernel32.SPTWith512Buffer>();
        buffer = Marshal.AllocHGlobal(length);
        Marshal.StructureToPtr(sptwb, buffer, false);

        validTransfer = Kernel32.DeviceIoControl(hDevice, Kernel32.Command.IOCTL_SCSI_PASS_THROUGH, buffer, length, buffer, length, out _, IntPtr.Zero);
        if (validTransfer) {
    ...
    public static SafeHandle IdentifyDevice(StorageInfo storageInfo) {
      ...
      buffer = Marshal.AllocHGlobal(length);
      Marshal.StructureToPtr(sptwb, buffer, false);
      var validTransfer = Kernel32.DeviceIoControl(handle, Kernel32.Command.IOCTL_SCSI_PASS_THROUGH, buffer, length, buffer, length, out _, IntPtr.Zero);
      Marshal.FreeHGlobal(buffer);
      ...
      if (validTransfer) {
        ...
        validTransfer = Kernel32.DeviceIoControl(handle, Kernel32.Command.IOCTL_SCSI_PASS_THROUGH, buffer, length, buffer, length, out _, IntPtr.Zero);
        if (validTransfer) {
          Marshal.PtrToStructure<Kernel32.SPTWith512Buffer>(buffer);
          Marshal.FreeHGlobal(buffer);
        } else {
          Marshal.FreeHGlobal(buffer);
          handle.Close();
